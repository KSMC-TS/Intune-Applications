...
$appname = "Remote Desktop" #needs to match Uninstall DisplayName in Registry
$appurl = "https://go.microsoft.com/fwlink/?linkid=2068602" #url to pull app from (GitHub, Azure Blob, etc.)
$addtlargs = "ALLUSERS=1" #any additional args needed for install command
...
if($Mode -eq "Install") {
    ...
    $(
        ...
        Write-Output "Creating Startup Shortcut: $startupshortcut"
        ...
        Write-Output "Running App for first time: $appfile"
        Start-Process -FilePath $appfile
        
        Write-Output "Install Complete!"
        Write-Output "############################################################"
    ) *>&1 >> $InstallCommand.ExitCode
    Exit 0
}
