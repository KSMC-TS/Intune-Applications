...
$appname = "Acrobat Reader" #needs to match Uninstall DisplayName in Registry
$appurl = "https://get.adobe.com/reader/webservices/json/standalone/?platform_type=Windows&platform_dist=Windows%2010&platform_arch=&language=English&eventname=readerotherversions" #url to pull app from (GitHub, Azure Blob, etc.)
$addtlargs = "/sAll EULA_ACCEPT=YES" #any additional args needed for install command
...
if($Mode -eq "Install") {
    ...
    $(
        ...
        ## custom Install script - different from standard template ##
        $headers = @{
            "Host"="get.adobe.com"
        ...
        $jsonRequest = Invoke-WebRequest $appurl -Headers $headers -UseBasicParsing | ConvertFrom-Json
        $downloadUrl = $jsonRequest.download_url
        Invoke-WebRequest $downloadUrl -OutFile $Path\$Installer
        ## end custom install script section ##
